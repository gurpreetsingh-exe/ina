...
let find_ty (ty_env : env) ident : ty option =
  if Hashtbl.mem ty_env.bindings ident
  then Some (Hashtbl.find ty_env.bindings ident)
  else None
;;
...
let add_binding (infer_ctx : infer_ctx) id ty =
  if Hashtbl.mem infer_ctx.ty_env.bindings id
  then Hashtbl.replace infer_ctx.ty_env.bindings id ty
...
let equate infer_ctx t0 t1 =
  if not (Hashtbl.mem infer_ctx.int_unifiction_table t0)
  then Hashtbl.add infer_ctx.int_unifiction_table t0 t1;
  if not (Hashtbl.mem infer_ctx.int_unifiction_table t1)
  then Hashtbl.add infer_ctx.int_unifiction_table t1 t0;
  Hashtbl.iter
    (fun k v ->
      match k with
      | Infer _ ->
          (try
             ...
             if found_v = v && found_k = k
             then Hashtbl.remove infer_ctx.int_unifiction_table v
           with
           | Not_found -> ())
...
let rec resolve_block (infer_ctx : infer_ctx) body =
  let rec sub ty =
    ...
    | Infer ((IntVar _ | FloatVar _) as i) ->
        sub
          (if Hashtbl.mem infer_ctx.int_unifiction_table ty
           then Hashtbl.find infer_ctx.int_unifiction_table ty
           else
             match i with
             | IntVar _ -> Int I64
    ...
    | FnTy (args_ty, ret_ty, is_variadic) ->
        ...
        then
          for i = 0 to expr_len - 1 do
            if i < ty_len
            then check (List.nth exprs i) (List.nth args_ty i)
            else ignore (infer infer_ctx (List.nth exprs i))
          done
        else if expr_len <> ty_len
        then (
          check_args exprs;
          infer_err_emit
            infer_ctx.tcx
...
and rec_replace (infer_ctx : infer_ctx) k ty =
  match k with
  | Infer _ ->
      if Hashtbl.mem infer_ctx.int_unifiction_table k
      then
...
and unify (infer_ctx : infer_ctx) (ty : ty) (expected : ty)
  ...
  | Ptr t ->
      (match expected with
       ...
       | _ -> Some (MismatchTy (expected, ty)))
  | ty ->
      if ty_neq infer_ctx.tcx ty expected
      then Some (MismatchTy (expected, ty))
      else None
...
let infer_func (infer_ctx : infer_ctx) (func : func) =
  ...
  | Some body ->
      ...
      if infer_ctx.tcx.sess.options.display_type_vars
      then print_endline (render_fn func);
