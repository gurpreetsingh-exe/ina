...
let emit emitter diagnostic =
  ...
  then
    ...
    | Some sm ->
        ...
        Styled_buffer.append buf ~line:!curr_line (pad ^ " |") LineNum;
        incr curr_line;
        let src = file#lookup_line_src line in
        let pad = String.make (max_line_num + 1) ' ' in
